...
app.use(session({
  ...
  cookie: {
    ...
    httpOnly: true,
    // Railway terminates SSL at load balancer and forwards via HTTP internally.
    // With trust proxy=1, Express sees the correct protocol.
...
app.get('/api/debug/session', (req, res) => {
  res.json({ sessionId: req.sessionID, userId: req.session.userId || null, cookie: req.session.cookie });
});
