  ...
  grid.innerHTML = list.map(c => {
    ...
    return `
      <div class="card" onclick="openCampaignModal(${c.id})">
        ...
        <p class="card-text">${c.desc.length > 120 ? c.desc.slice(0,117)+'…' : c.desc}</p>
...
async function openCampaignModal(id) {
  ...
  document.getElementById('modal-content').innerHTML = `
    ...
    <p style="color:var(--text-dim);line-height:1.7;margin-bottom:1.5rem;">${c.desc}</p>
...
async function doCreateCampaign() {
  ...
  try {
    const data = await API.post('/campaigns', { name, desc, level, status });
